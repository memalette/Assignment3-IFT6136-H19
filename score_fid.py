# -*- coding: utf-8 -*-
"""
Created on Fri Apr 19 09:53:21 2019
...
def calculate_fid_score(sample_feature_iterator,
    ...
    """
    sample_f = np.zeros((1000,512))
    test_f = np.zeros((1000,512))
    k = 0
    for sam, test in zip(sample_feature_iterator, testset_feature_iterator):
       sample_f[k,:] = sam
       test_f[k,:] = test
       k += 1

    mu_q    = np.mean(sample_f, axis=0)
    Sigma_q = np.cov(sample_f, rowvar=False)
    mu_p    = np.mean(test_f, axis=0)
    Sigma_p = np.cov(test_f, rowvar=False)
    
    mu_n = mu_p - mu_q
    
    l_term, _ = linalg.sqrtm(Sigma_p.dot(Sigma_q), disp=False)
    l_term = l_term.real    
    
    return mu_n.dot(mu_n) + np.trace(Sigma_p)+np.trace(Sigma_q) - 2 * np.trace(l_term)
